...
class GoalsRepository:
    ...
    async def put_milestones_and_steps(
        ...
        await self._table.update_item(
            key={"user_id": user_id, "id": goal_id},
            update_expression=(
                F("milestones").set(json.dumps([m.model_dump() for m in milestones]))
                & F("steps").set(json.dumps([s.model_dump() for s in steps]))
            ),
            return_values=ReturnValues.none,
        )
    ...
    async def get_milestones(self, goal_id: str, user_id: str) -> list[Milestone] | None:
        ...
        if item is None:
            return None
        raw = item.get("milestones")
        if not raw:
            return []
        return [Milestone(**m) for m in json.loads(raw)]
    ...
    async def get_steps(self, goal_id: str, user_id: str) -> list[Step] | None:
        ...
        if not raw:
            return []
        return [Step(**s) for s in json.loads(raw)]
